...
import type {
  SupplierInviteHistoryEntry,
  SupplierInviteResult,
  SupplierOperationalUserRow,
...
export function SupplierAccessPanel({
  ...
  defaultEmail: string;
  initialUsers: SupplierOperationalUserRow[];
  initialHistory: SupplierInviteHistoryEntry[];
  organizationName: string;
}) {
  const router = useRouter();
  const [email, setEmail] = useState(defaultEmail);
  const [busy, setBusy] = useState(false);
  const [result, setResult] = useState<SupplierInviteResult | null>(null);
  ...
  async function invite() {
    ...
    try {
      const res = await fetch(
        `/api/admin/supplier-applications/${applicationId}/invite`,
        {
          method: "POST",
  ...
  return (
    <div className="mt-4 rounded-md border border-slate-800 bg-slate-950/40 p-4">
      ...
      >
        <label className="flex-1">
          ...
          <input
            ...
            placeholder="ops@supplier.example"
      ...
      <p className="mt-2 font-mono text-[10.5px] uppercase tracking-[0.14em] text-slate-500">
        Provisions a supplier_admin user for {organizationName}. If email/auth
        is not wired in this environment, the invite is recorded as a placeholder.
      </p>
      ...
      <div className="mt-5">
        ...
        {initialUsers.length === 0 ? (
          <p className="text-xs text-slate-500">
            No users yet — invite the supplier above.
          </p>
        ) : (
...
function InviteResultBanner({ result }: { result: SupplierInviteResult }) {
